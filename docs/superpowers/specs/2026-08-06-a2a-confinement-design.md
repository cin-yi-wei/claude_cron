...
## 威脅模型與這些等級真正買到的東西（2026-08-07 補記）

本規格實作完成後，operator 確認了一件會改變其定位的事實：**發起委派的只有 operator 自己的 `cc-` binding**，沒有外部呼叫方。

`cc-` binding 本來就以 operator 的身分在這台主機上無限制執行。一個會惡意委派的 `cc-` session 不需要繞道 A2A——它直接做就好，連 permission gate 都不會經過。因此本規格所建立的約束，**不是**在防一個有敵意的呼叫方。

而在同一個 unix 使用者底下，`develop` 與 `full` 之間也沒有真正的牆：政策檔與沙盒同屬一個使用者，而 `develop` 依設計就允許執行 `python` / `node` / `go run`，所以它可以改寫自己的政策檔把自己升成 `full`（已實測：改寫後 `curl` 與 `mcp__*` 由 deny 變 allow）。這不是可以補的洞，是「同一個使用者身分」這個前提本身的結果——把 `sed` 從允許清單拿掉沒有意義，`python` 一行就做得到。

因此，這三個等級的正確定位是：

- **`readonly` 是唯一真正的邊界**，而且它是實的。最終安全 review 對它實跑了約 45 條攻擊指令（`rg --pre`、`find -exec/-delete`、`git diff --output`、`git branch`、環境變數前綴、群集短旗標、`git -c core.pager`、NAT64／6to4／SIIT 位址編碼等），沒有一條能執行程式、寫入檔案或連出網路。
- **`develop` 與 `full` 之間是安全帶，不是牆。** 它擋得住手滑（路徑寫錯、`rm` 到別的地方、不小心碰到別的專案），擋不住蓄意。授予 `develop` 時，心裡應當視同授予 `full`。

要讓 `develop` 成為真正的邊界，需要作業系統層的隔離（另開 unix 使用者，或容器）。容器方案已完整評估並驗證過關鍵機制，因成本效益在「只有自己人」的模型下不成立而暫停保留，見 `2026-08-07-a2a-container-isolation-STATUS.md`。**若將來開放外部呼叫方，這一節的結論全部反轉，該文件可直接接續。**
